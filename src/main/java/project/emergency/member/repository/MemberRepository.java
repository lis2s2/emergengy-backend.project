...
public interface MemberRepository extends JpaRepository<Member, String> {
    ...
    //    boolean existsByPwd(String memPwd); // 비밀번호 찾기
    @Query("SELECT m.memPwd FROM Member m WHERE m.memId = :memId AND m.memName = :memName AND m.memEmail = :memEmail")
    String findPwd(@Param("memId") String memId, @Param("memName") String memName, @Param("memEmail") String memEmail);


//    Optional<Member> findByMemPwd(String memPwd);
//    String findPassword(String memId, String memName, String memEmail);
    // 비밀번호 찾기 관련 쿼리
